...
The class names match exactly what you'd find inspecting the popup with your browser's dev tools, written as either `snake_case` or the class's own hyphenated form. A handful of the most useful ones: `ha-card`, `title`, `state`, `percentage`, `last-changed`, `slider`, `handle`, `main-control`, `control-button-group`, `control-button`, `icon-button-group`, `icon-toggle-button`, `tooltip`, `favorites`, `favorite-button`. The popup window around all of this - the backdrop, the dialog box, the header - has its own classes too, styled the exact same way: `overlay`, `frame`, `header`, `heading`, `close-button`, `body`.
...
You can nest classes inside each other to reach an element that sits inside another one, the same way you'd write a CSS selector like `.slider .handle`:
...
This styles the slider itself, and separately styles the handle inside it. You don't have to nest - styling `handle` directly at the top level works exactly the same, since every element already has its own class. Nesting is just another way to write it, useful when it reads more clearly grouped together.
...
| `--slider-border-radius` | `slider` | `36px` | How rounded the slider's own outer corners are. |
| `--slider-track-bar-border-radius` | `slider` | `8px` | How rounded the corners of the filled bar inside the slider are. Kept independent of `--slider-border-radius` so the fill doesn't distort into a flattened dome shape at low percentages. |
| `--handle-size` | `slider` | `4px` | The thickness of the white handle bar. |
| `--handle-color` | `handle` | `white` | The color of the handle bar. |
...
| `--icon-button-group-max-width` | `icon-button-group` | `96px` | Widest the slider/buttons toggle pill is allowed to grow to. |
...
**Popup window variables** (these size and color the dialog frame itself, around the content above):

| Variable | Set it under | Default | What it changes |
| :--- | :--- | :--- | :--- |
| `--chrono-cover-popup-z-index` | `overlay` | `10000` | Stacking order of the popup above the rest of the page. |
| `--chrono-cover-popup-backdrop` | `overlay` | `rgba(0, 0, 0, 0.5)` | Color of the dimmed background behind the popup. |
| `--chrono-cover-popup-max-width` | `frame` | `580px` | Maximum width of the popup dialog. |
| `--chrono-cover-popup-margin-top` | `frame` | `10vh` | Space above the popup dialog. |
| `--chrono-cover-popup-background` | `frame` | Your theme's card background color | Background color of the popup dialog itself. |
| `--chrono-cover-popup-border-radius` | `frame` | `24px` | Corner rounding of the popup dialog. |
| `--chrono-cover-popup-box-shadow` | `frame` | `0 8px 32px rgba(0, 0, 0, 0.5)` | Drop shadow around the popup dialog. |
